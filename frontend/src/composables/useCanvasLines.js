...
function pLine(x1, y1, x2, y2) {
  const p = new Path2D(); p.moveTo(x1, y1); p.lineTo(x2, y2); return p;
}
...
function drawHLPath(ctx, p, color, alphaMult) {
  w(ctx, () => {
    ...
    ctx.strokeStyle = color; ctx.lineWidth = SYS_W; ctx.lineCap = 'round';
    ctx.stroke(p);
  });
}
...
export function useCanvasLines(
  ...
  hiddenTracks, interactionState, callbacks
) {
  ...
  function executeDrawGraph(ctx, W, H, vMode, layout, bg, hidden) {
    ...
    connections.value.forEach(c => {
      const s = schedules.value.find(x=>x.id===c.from)
      const e = schedules.value.find(x=>x.id===c.to)
      if (!s || !e) return
      ...
      edges.push({ id: `e_${s.id}_${e.id}`, from: s, to: e, track: s.track, targetTrack: e.track, color, isCross })
    })

    sorted.forEach(t => {
      if (!isSys(t.id)) return    
      if (isHidden(t.id)) return  
      ...
      for (let i = 0; i < ns.length - 1; i++) {
        ...
        if (!alreadyExists) {
          edges.push({ 
            id: `e_${ns[i].id}_${ns[i+1].id}`, from: ns[i], to: ns[i+1], track: t.id, targetTrack: t.id, 
            color: t.color, isCross: false 
          })
        }
      }
    })
    ...
    Object.values(branchesTo).forEach(branches => {
      branches.forEach(b => {
        b.sIdx = getTrackIdx(b.track); b.tIdx = getTrackIdx(b.targetTrack); b.dist = Math.abs(b.sIdx - b.tIdx)
      })
    ...
    const getAlphaMult = (itemType, item) => {
      ...
      if (hovered) {
        ...
        if (clicked.type === 'node') {
          ...
          if (itemType === 'edge') {
            isHL = (item.track === cTrack && item.targetTrack === cTrack) || item.from.id === cId || item.to.id === cId;
          }
          if (itemType === 'node') {
    ...
    for (let r=0; r<totalRows; r++) {
      ...
      edges.forEach(edge => {
        ...
        const alphaMult = getAlphaMult('edge', edge);
        const finalAlpha = LINE_ALPHA * alphaMult;
        let path1, path2;

        if (!edge.isCross) {
          path1 = pLine(sx, sy, ex, sy);
          hitPaths.push({ edge, path: path1 });
          ...
          if (!isTargetInRow) {
            ...
            const addDraw = (p) => {
              if (alphaMult < 1) L1.push(() => drawPath(ctx, p, edge.color, baseLineW, finalAlpha, false));
                ...
                L2_gaps.push(() => drawPath(ctx, p, edge.color, baseLineW, 1, true));
                L2_draws.push(() => drawPath(ctx, p, edge.color, baseLineW, finalAlpha, false));
              }
            }

            if (Math.abs(ex - sx) < CW * 0.85 && sx >= 0 && sx <= W) {
              const bypassX = Math.round(Math.max(sx, ex) + CW * 0.4 + ib * 10)
              path1 = pDetour(sx, sy, ex, finalEy, bypassX);
              hitPaths.push({ edge, path: path1 }); addDraw(path1);
            } else if (isSys(edge.targetTrack)) {
              const px = Math.round(ex - PLUG_PX - ib * 10)
              path1 = pLine(sx, sy, px, sy);
              path2 = pBend(px, sy, ex, finalEy, px);
              hitPaths.push({ edge, path: path1 }); hitPaths.push({ edge, path: path2 });
              L1.push(() => drawPath(ctx, path1, edge.color, baseLineW, finalAlpha));
              addDraw(path2);
            } else {
              path1 = pBend(sx, sy, ex, finalEy, bx);
              hitPaths.push({ edge, path: path1 }); addDraw(path1);
            }
          }
        }
    ...
    schedules.value.forEach(s => {
      ...
      const alphaMult = getAlphaMult('node', s);
      L3.push(() => rDot(ctx, x, y, t.color, bg, alphaMult))
    })
  ...
  const getMousePos = (e) => {
    const cvs = lineCanvas.value;
    if (!cvs) return null;
    const rect = cvs.getBoundingClientRect();
    const scaleX = cvs.width / rect.width;
    const scaleY = cvs.height / rect.height;
    return { x: (e.clientX - rect.left) * scaleX, y: (e.clientY - rect.top) * scaleY, cvs };
  }
  ...
  const handleMouseMove = (e) => {
    ...
    if (e.target.closest('.schedule-dot, .week-node, .schedule-label-chip, .ptl-header-track-badge')) {
    ...
    for (let i = currentHitPaths.length - 1; i >= 0; i--) {
      if (ctx.isPointInStroke(currentHitPaths[i].path, pos.x, pos.y)) {
        foundEdge = currentHitPaths[i].edge; break;
      }
    }
  ...
  const handleMouseClick = (e) => {
    if (e.target.closest('.schedule-dot, .week-node, .schedule-label-chip')) return;
